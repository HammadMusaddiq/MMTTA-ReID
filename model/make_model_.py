...
from model.nomic_backbones_ import FrozenNomicVision, FrozenNomicText
...
class build_transformer(nn.Module):
    ...
    def forward(self, x1, x2, x3, label=None, cam_label= None, view_label=None, captions=None):
        ...
        # teacher CLS (optional, may be None) # updated Ham
        # if self.teacher_vis:
        #     with torch.no_grad():
        #         # t_raw = self.teacher_vis(x1)                    # (B,1024 or 768 ...)
        #         t_raw = self.teacher_vis(x1).detach()                    # (updated)
