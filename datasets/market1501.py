...
class Market1501(BaseImageDataset):
    ...
    def __init__(self, root='', verbose=True, i_modality=None, c_modality=None, **kwargs):
        ...
        train = self._process_dir_train(self.train_dir, prefix=self.train_caption_prefix, relabel=True)
        query = self._process_dir_test(self.query_dir, prefix=self.query_caption_prefix, relabel=False)
        gallery = self._process_dir_test(self.gallery_dir, prefix=self.test_caption_prefix, relabel=False)
    ...
    def _process_dir_train(self, dir_path, prefix, relabel=False):
    ...
    def _process_dir_test(self, dir_path, prefix, relabel=False):
        img_paths_RGB = glob.glob(osp.join(dir_path, 'visible', '*.jpg'))
        pid_container = set()
