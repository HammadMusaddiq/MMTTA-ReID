...
from .multi_modal_margin_loss_new import multiModalMarginLossNew, multiModalMarginLossNew_dual
from .hard_mine_triplet_loss_multi_modal import MMTripletLoss
from .multi_modal_id_margin_loss import IDMarginLossNew, IDMarginLossNew_dual
...
#     caption_modalities = cfg.CAPTION.MODALITY if caption_strategy == "matched" else ["RGB"]
#     cap_triplet = TripletLoss(cfg.SOLVER.MARGIN)
#     cap_contrastive = ContrastiveLoss(margin=0.3)
#     cap_weight = cfg.CAPTION.LOSS.WEIGHT
...
#             if captions is not None and isinstance(feat, list) and len(feat) > 4:
...
def make_loss_dual(cfg, num_classes):    # modified by gu
    sampler = cfg.DATALOADER.SAMPLER
    feat_dim = 2048
    Hint = nn.MSELoss()

    # Two Modalities
    criterion_m = multiModalMarginLossNew_dual(margin=1)
    criterion_i = IDMarginLossNew_dual(margin=0.5, dist_type='l1')
        ...
        def loss_func(score, feat, target, target_cam):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    ...
                    if isinstance(feat, list):
                            # Two Modality
                            IDM_LOSS = criterion_i(F.normalize(feat[1], p=2, dim=1), F.normalize(feat[2], p=2, dim=1), target)
                            MMM_LOSS = criterion_m(F.normalize(feat[1], p=2, dim=1), F.normalize(feat[2], p=2, dim=1), target)
                            TRI_LOSS = 0.5 * MMM_LOSS + 0.5 * IDM_LOSS + triplet(feat[0], target)[0]
