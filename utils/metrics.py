...
class R1_mAP_eval():
    ...
    #     id = []
    #     id.append(pid[0].index(max(pid[0])))
    #     id.append(pid[1].index(max(pid[1])))
    #     print(id)
    ...
    def compute(self):  # called after each epoch
        # print('len(feats)', len(self.feats))
        ...
        print('qf.num, gf.num',len(qf), len(gf))
        print('qf.shape, gf.shape',qf[0].shape, gf[0].shape)
        if self.reranking:
